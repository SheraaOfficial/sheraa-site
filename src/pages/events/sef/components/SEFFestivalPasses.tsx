...
const PassCard: React.FC<{ pass: PassType; index: number }> = ({ pass, index }) => {
  return (
    <motion.div 
      className={`relative rounded-xl overflow-hidden border ${pass.popular ? 'border-purple-400/50' : 'border-gray-200/30'} shadow-lg`}
      ...
      <div className="p-6 bg-white dark:bg-gray-800">
        ...
        <div className="mt-8">
          {pass.popular ? (
            ...
            <Button variant={pass.name === "Executive Pass" ? "secondary" : "outline"} className="w-full">
              Get Your Pass
            </Button>
          )}
        </div>
      ...
      <div className="container mx-auto max-w-6xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {passes.map((pass, index) => (
            <PassCard key={pass.name} pass={pass} index={index} />
          ))}
        </div>
