...
const EligibilityChecker = () => {
  ...
  // Apply spring physics with adjusted parameters for mobile
  const springScroll = useSpring(scrollYProgress, {
    stiffness: isMobile ? 100 : 60,
    damping: isMobile ? 30 : 15,
  });

  // Transform values - simplified for mobile
  const backgroundOpacity = useTransform(
    springScroll,
    [0, 0.5, 1],
    isMobile ? [0.8, 0.9, 0.8] : [0.7, 1, 0.7]
  );
  
  const backgroundScale = isMobile ? 1 : useTransform(
    springScroll,
    [0, 0.5, 1],
    [0.95, 1.05, 0.95]
  );
  
  const xOffset = isMobile ? 0 : useTransform(
    springScroll,
    [0, 0.5, 1],
    [-20, 0, 20]
  );
  ...
  return (
    <section className="py-12 md:py-20 bg-white relative overflow-hidden" ref={containerRef}>
      <div className="container mx-auto px-4 relative z-10">
        ...
        >
          {/* Background gradient blobs - simplified for mobile */}
          {!isMobile && (
            <motion.div
