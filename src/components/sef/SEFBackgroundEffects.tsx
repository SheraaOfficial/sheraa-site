...
export const SEFBackgroundEffects: React.FC = () => {
  ...
  return <>
      ...
      <div className="absolute inset-0 bg-gradient-to-br from-[#6E59A5] to-[#1A1F2C] overflow-hidden rounded-3xl z-0">
        {/* Floating orbs */}
        {orbs.map((orb, i) => <motion.div key={i} className="absolute rounded-full blur-2xl opacity-40" style={{
        left: `${orb.x}%`,
        top: `${orb.y}%`,
        width: `${orb.size}px`,
        height: `${orb.size}px`,
        ...
        <AnimatedGradient colors={['#8B5CF6', '#9b87f5', '#D6BCFA']} speed={8} blur="medium" className="opacity-20" />

        <Particles className="absolute inset-0" quantity={100} staticity={40} color="#ffffff" ease={60} size={0.5} />
      </div>
      ...
      {/* Glowing effect */}
      <div className="absolute inset-0 z-[2] bg-violet-950">
      ...
      {/* Top border beam */}
      <BorderBeam className="z-20" colorFrom="#ffffff" colorTo="#9b87f5" size={350} duration={12} borderWidth={2} />
    </>;
};