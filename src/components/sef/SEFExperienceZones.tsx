...
export const SEFExperienceZones: React.FC<SEFExperienceZonesProps> = ({
  hasRevealed,
  isInView
}) => {
  ...
  return <div className="pt-8">
      ...
      duration: 0.6
    }} className="font-bold mb-5 text-white drop-shadow-glow text-4xl">
        SEF'26 Experience
      </motion.h3>
      <motion.div className="grid grid-cols-1 sm:grid-cols-2 gap-4" variants={containerVariants} initial="hidden" animate={hasRevealed ? "visible" : "hidden"}>
        {experienceZones.map((zone, index) => <motion.div key={index} variants={itemVariants} transition={{
        duration: 0.7,
        type: "spring",
        stiffness: 50
      }}>
            <Card className="border-white/30 hover:border-white/60 backdrop-blur-xl bg-white/20 hover:bg-white/30 
