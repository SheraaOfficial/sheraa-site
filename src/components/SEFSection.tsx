...
const SEFSection = () => {
  ...
  const stats = [
    ...
    { value: "300+", label: "Global Speakers", icon: Mic },
    { value: "250+", label: "Activities", icon: Calendar },
    { value: "10+", label: "Zones", icon: Layout }
  ];
  
  const experienceZones = [
    { name: "STARTUP TOWN", description: "Building the Future Together", icon: Star },
    { name: "MADE IN SHARJAH", description: "Honoring Local Talent", icon: Award },
    { name: "INVESTORS LOUNGE", description: "Connect with Capital", icon: Users },
    { name: "SEF ACADEMY", description: "Learning at Every Level", icon: BookOpen },
    { name: "PITCH COMPETITION", description: "Showcase Your Ideas", icon: Mic },
    { name: "IMPACT ZONE", description: "Where Inspiration Meets Purpose", icon: Star },
  ];
        ...
        <div className="container mx-auto px-4 md:px-8 py-16">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
            ...
            <div className="space-y-8">
              ...
              <h2 
                className={`text-2xl md:text-3xl lg:text-4xl font-bold text-sheraa-dark transition-all duration-700 delay-100 ${
                  isInView ? "opacity-100 transform-none" : "opacity-0 translate-y-4"
                }`}
              >
              ...
              <div className="pt-6">
                <h3 className={`text-xl font-semibold mb-4 transition-all duration-700 delay-400 ${
                  isInView ? "opacity-100 transform-none" : "opacity-0 translate-y-4"
                }`}>
                  SEF 2026 Experience
                </h3>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {experienceZones.map((zone, index) => (
                    <Card 
                      key={index}
                      className={`border-sheraa-primary/10 hover:border-sheraa-primary/30 transition-all group hover:shadow-md transform hover:translate-y-[-2px] ${
            ...
            <div className="space-y-8">
              {/* Event Card */}
              <Card 
                className={`overflow-hidden shadow-lg border-sheraa-primary/20 transition-all duration-1000 ${
              ...
              >
                ...
                <CardHeader className="pb-2">
                  ...
                  <CardDescription>Where entrepreneurs & investors connect</CardDescription>
                </CardHeader>
              ...
              <div>
                ...
                >
                  {stats.map((stat, index) => (
                    <Card 
                      key={index}
                      className={`border-sheraa-primary/10 hover:border-sheraa-primary/30 transition-all transform hover:translate-y-[-2px] hover:shadow-md ${
                ...
                <div className="flex flex-wrap gap-2">
                  {["Agenda", "Speakers", "Experience", "Who Should Attend", "Be Part of SEF"].map((link, index) => (
                    <Link 
                      ...
                      className="px-3 py-1.5 bg-gray-100 hover:bg-sheraa-primary/10 text-gray-700 hover:text-sheraa-primary rounded-full text-sm transition-all"
                    >
                      {link}
                    </Link>
                  ))}
