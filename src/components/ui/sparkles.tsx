...
const SparkleInstance = memo(({ sparkle }: { sparkle: SparkleType }) => (
  <span
...
export const Sparkles: React.FC<SparklesProps> = ({
  ...
  count = 10, // Reduced from 20 to improve performance
  minSize = 10,
  maxSize = 20,
  speed = 1000, // Increased to reduce updates
  className,
  children,
}) => {
  ...
  const generateSparkles = (num: number): SparkleType[] => {
    return Array.from({ length: num }, () => generateSparkle());
  };
